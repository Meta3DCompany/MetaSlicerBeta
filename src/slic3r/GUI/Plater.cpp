...
int varky = 0;
wxTimer* waitingTimer = nullptr;
int dotCount = 0;
bool growingDots = true;
...
class QualityObserver: public IObserver , public ConfigOperations{
...
private:
    ...
    double layer_height;
    double slice_gap;
    double slice_closing_radius;
    double resolution;
    float bridge_speed;
    bool enable_overhang_speed;
    float gap_infill_speed;
    float overhang_1_4_speed;
    float overhang_2_4_speed;
    float overhang_3_4_speed;
    float overhang_4_4_speed;
    float initial_layer_infill_speed;
    float initial_layer_speed;
    float initial_layer_travel_speed;
    float inner_wall_speed;
    float internal_solid_infill_speed;
    float ironing_speed;
    float outer_wall_speed;
    bool overhang_speed_classic;
    bool role_based_wipe_speed;
    float small_perimeter_speed;
    float sparse_infill_speed;
    float support_interface_speed;
    float support_speed;
...
public:
    void Update(const std::string& botResponse) override {
        ...
        if(botResponse =="increase_quality()")
        {
            
           
            DynamicPrintConfig preset_config = *wxGetApp().get_tab(Preset::TYPE_PRINT)->m_config;
            const DynamicPrintConfig& printer_config = wxGetApp().preset_bundle->printers.get_edited_preset().config;
            outer_wall = preset_config.get_abs_value("outer_wall_line_width");
            inner_wall = preset_config.get_abs_value("inner_wall_line_width");
            layer_height = preset_config.get_abs_value("layer_height");
            slice_closing_radius = preset_config.get_abs_value("slice_closing_radius");
            resolution = preset_config.get_abs_value("resolution");
            //double nozzle_diameter = printer_config.option<ConfigOptionFloats>("nozzle_diameter")->get_at(0);
            bridge_speed = preset_config.get_abs_value("bridge_speed");
            //enable_overhang_speed = preset_config.get_abs_value("enable_overhang_speed");
            gap_infill_speed = preset_config.get_abs_value("gap_infill_speed");
            overhang_1_4_speed = preset_config.get_abs_value("overhang_1_4_speed");
            overhang_2_4_speed = preset_config.get_abs_value("overhang_2_4_speed");
            overhang_3_4_speed = preset_config.get_abs_value("overhang_3_4_speed");
            overhang_4_4_speed = preset_config.get_abs_value("overhang_4_4_speed");
            initial_layer_infill_speed = preset_config.get_abs_value("initial_layer_infill_speed");
            initial_layer_speed = preset_config.get_abs_value("initial_layer_speed");
            initial_layer_travel_speed = preset_config.get_abs_value("initial_layer_travel_speed");
            inner_wall_speed = preset_config.get_abs_value("inner_wall_speed");
            internal_solid_infill_speed = preset_config.get_abs_value("internal_solid_infill_speed");
            ironing_speed = preset_config.get_abs_value("ironing_speed");
            outer_wall_speed = preset_config.get_abs_value("outer_wall_speed");
            //overhang_speed_classic = preset_config.get_abs_value("overhang_speed_classic");
            //role_based_wipe_speed = preset_config.get_abs_value("role_based_wipe_speed");
            small_perimeter_speed = preset_config.get_abs_value("small_perimeter_speed");
            sparse_infill_speed = preset_config.get_abs_value("sparse_infill_speed");
            support_interface_speed = preset_config.get_abs_value("support_interface_speed");
            support_speed = preset_config.get_abs_value("support_speed");
            top_surface_speed = preset_config.get_abs_value("top_surface_speed");
            travel_speed = preset_config.get_abs_value("travel_speed");
            travel_speed_z = preset_config.get_abs_value("travel_speed_z");
            wipe_speed = preset_config.get_abs_value("wipe_speed");
         
            outer_wall = outer_wall - (outer_wall * (0.35));
            inner_wall = inner_wall - (inner_wall * (0.20));
            layer_height =layer_height - (layer_height * (0.25));
            slice_closing_radius = slice_closing_radius - (slice_closing_radius * (0.10));
            resolution =resolution - (resolution * (0.10));
            bridge_speed = bridge_speed - (bridge_speed * (0.10));
            //bool enable_overhang_speed;
            gap_infill_speed =gap_infill_speed - (gap_infill_speed * (0.10));
            overhang_1_4_speed = overhang_1_4_speed - (overhang_1_4_speed * (0.10));
            overhang_2_4_speed = overhang_2_4_speed - (overhang_2_4_speed * (0.10));
            overhang_3_4_speed = overhang_3_4_speed - (overhang_3_4_speed * (0.10));
            overhang_4_4_speed = overhang_4_4_speed - (overhang_4_4_speed * (0.10));
            initial_layer_infill_speed = initial_layer_infill_speed - (initial_layer_infill_speed * (0.10));
            initial_layer_speed = initial_layer_speed - (initial_layer_speed * (0.10));
            initial_layer_travel_speed = initial_layer_travel_speed - (initial_layer_travel_speed * (0.10));
            inner_wall_speed = inner_wall_speed - (inner_wall_speed * (0.10));
            internal_solid_infill_speed = internal_solid_infill_speed - (internal_solid_infill_speed * (0.10));
            ironing_speed = ironing_speed - (ironing_speed * (0.10));
            outer_wall_speed = outer_wall_speed - (outer_wall_speed * (0.10));
            //bool overhang_speed_classic;
            //bool role_based_wipe_speed;
            small_perimeter_speed = small_perimeter_speed - (small_perimeter_speed * (0.10));
            sparse_infill_speed = sparse_infill_speed - (sparse_infill_speed * (0.10));
            support_interface_speed =support_interface_speed - (support_interface_speed * (0.10));
            support_speed = support_speed - (support_speed * (0.10));
            top_surface_speed =top_surface_speed - (top_surface_speed * (0.10));
            travel_speed = travel_speed - (travel_speed * (0.10));
            travel_speed_z =travel_speed_z - (travel_speed_z * (0.10));
            wipe_speed = wipe_speed - (wipe_speed * (0.10));
            
            preset_config.set_key_value("outer_wall_line_width", new ConfigOptionFloatOrPercent(outer_wall, false));
            preset_config.set_key_value("inner_wall_line_width", new ConfigOptionFloatOrPercent(inner_wall, false));
            preset_config.set_key_value("layer_height", new ConfigOptionFloat(layer_height));
            preset_config.set_key_value("slice_closing_radius", new ConfigOptionFloat(slice_closing_radius));
            preset_config.set_key_value("resolution", new ConfigOptionFloat(resolution));
            preset_config.set_key_value("bridge_speed", new ConfigOptionFloat(bridge_speed));
            preset_config.set_key_value("gap_infill_speed", new ConfigOptionFloat(gap_infill_speed));
            preset_config.set_key_value("overhang_1_4_speed", new ConfigOptionFloatOrPercent(overhang_1_4_speed, false));
            preset_config.set_key_value("overhang_2_4_speed", new ConfigOptionFloatOrPercent(overhang_2_4_speed, false));
            preset_config.set_key_value("overhang_3_4_speed", new ConfigOptionFloatOrPercent(overhang_3_4_speed, false));
            preset_config.set_key_value("overhang_4_4_speed", new ConfigOptionFloatOrPercent(overhang_4_4_speed, false));
            preset_config.set_key_value("initial_layer_infill_speed", new ConfigOptionFloat(initial_layer_infill_speed));
            preset_config.set_key_value("initial_layer_speed", new ConfigOptionFloat(initial_layer_speed));
            preset_config.set_key_value("initial_layer_travel_speed", new ConfigOptionFloatOrPercent(initial_layer_travel_speed, false));
            preset_config.set_key_value("inner_wall_speed", new ConfigOptionFloat(inner_wall_speed));
            preset_config.set_key_value("internal_solid_infill_speed", new ConfigOptionFloat(internal_solid_infill_speed));
            preset_config.set_key_value("ironing_speed", new ConfigOptionFloat(ironing_speed));
            preset_config.set_key_value("outer_wall_speed", new ConfigOptionFloat(outer_wall_speed));
            //preset_config.set_key_value("overhang_speed_classic", new ConfigOptionFloatOrPercent(overhang_speed_classic, false));
            //preset_config.set_key_value("role_based_wipe_speed", new ConfigOptionFloatOrPercent(role_based_wipe_speed, false));
            preset_config.set_key_value("small_perimeter_speed", new ConfigOptionFloatOrPercent(small_perimeter_speed, false));
            preset_config.set_key_value("sparse_infill_speed", new ConfigOptionFloat(sparse_infill_speed));
            preset_config.set_key_value("support_interface_speed", new ConfigOptionFloat(support_interface_speed));
            preset_config.set_key_value("support_speed", new ConfigOptionFloat(support_speed));
            preset_config.set_key_value("top_surface_speed", new ConfigOptionFloat(top_surface_speed));
            preset_config.set_key_value("travel_speed", new ConfigOptionFloat(travel_speed));
            preset_config.set_key_value("travel_speed_z", new ConfigOptionFloat(travel_speed_z));
            preset_config.set_key_value("wipe_speed", new ConfigOptionFloatOrPercent(wipe_speed, false));
            wxGetApp().get_tab(Preset::TYPE_PRINT)->get_config_manipulation().apply(wxGetApp().get_tab(Preset::TYPE_PRINT)->m_config,&preset_config);
...
wxString ChatFrame::cleanBotResponse(const wxString& response) {
    std::string responseSTD = std::string(response.mb_str(wxConvUTF8));
        ...
        // Regex to match everything from the first dollar sign to the end of the string
        std::regex pattern(R"(\$.*$)");

        // Replace the matched pattern with an empty string
        std::string cleanedResponse = std::regex_replace(responseSTD, pattern, "");

        // Trim any leading/trailing spaces that might be left
...
void ChatFrame::OnWaitingTimer(wxTimerEvent& event) {
    
    chatDisplay->SetDefaultStyle(wxTextAttr(*wxWHITE));
    wxString dots = ".";
    dotCount++;
    chatDisplay->AppendText(dots);
...
void ChatFrame::OnSend(wxCommandEvent& event) {
    ...
    dotCount = 0;
    growingDots = true;  // Start by growing the dots
    chatDisplay->SetDefaultStyle(wxTextAttr(*wxRED));
    chatDisplay->AppendText("Bot: ");  // Add Bot: only once here
    chatDisplay->SetDefaultStyle(wxTextAttr(*wxWHITE));
    chatDisplay->AppendText("Support3D is thinking .");  // Add Bot: only once here
    
    waitingTimer = new wxTimer(this, wxID_ANY);
    Bind(wxEVT_TIMER, &ChatFrame::OnWaitingTimer, this);
    waitingTimer->Start(500);  // Update every 500ms

    // Simulate chatbot API call asynchronously
    std::thread([this, userText]() {
        wxString botResponse = CallChatbotAPI(userText);

        // Stop the waiting animation when the response is ready
        wxTheApp->CallAfter([this, botResponse, userText]() {
            if (waitingTimer) {
                waitingTimer->Stop();
                delete waitingTimer;
            ...
            // Remove the dots when the response is ready
            int lastPos = chatDisplay->GetLastPosition();
            chatDisplay->Remove(lastPos - dotCount-23  , lastPos);  // Remove the dots but keep "Bot:"
            
            std::cout << "Dot count is :"<< dotCount<<std::endl;
            chatDisplay->SetDefaultStyle(wxTextAttr(*wxWHITE));
            
            std::string entity, operation;
            ...
            wxString boolLanguage = wxString(queryAdditionalChatbot(std::string(userText.mb_str()), -2));
            
            if (boolLanguage == "Not-English" && ifRasa) {
                cleanedResponse = wxString(queryAdditionalChatbot(std::string(cleanedResponse.mb_str()), -1));
            }
...
std::string ChatFrame::queryAdditionalChatbot(const std::string& prompt, int chooseBot)
{
    if(chooseBot == 0)
    {
        
        std::cout << "Using OpenAI ChatGPT" << std::endl;
               CURL* curl;
               CURLcode res;
               std::string response_string;
               struct curl_slist* headers = NULL;
               curl = curl_easy_init();
               std::string generated_text = "Error: Unable to process the request.";
               
               if (curl) {
                   const std::string api_key = "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"; // Directly assigning the API key
                   const std::string api_url = "https://api.openai.com/v1/chat/completions"; // Update the URL for chat models
                   
                   // Create the JSON payload
                   nlohmann::json jsonData;
                   jsonData["model"] = "gpt-3.5-turbo"; // Use gpt-3.5-turbo model
                   
                   //std::string prompt =   prompt;  // Concatenate without modifying chatGPTReply
                  

                   jsonData["messages"] = {
                           {{"role", "system"}, {"content", "You are MetaSlicer, a slicer assistant created by Meta3D. You assist users with 3D printing tasks, troubleshooting MetaSlicer software, and optimizing 3D printing workflows. Provide responses that reflect Meta3D's professional tone. Respond no more than 100 words."}},
                           {{"role", "user"}, {"content", prompt}} // User input from 'prompt'
                       };
                   //jsonData["max_tokens"] = 100;
                   std::string json_payload = jsonData.dump();
                   ...
                   curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WriteCallback); // Define WriteCallback function elsewhere in your code
                   ...
                   if (res != CURLE_OK) {
                       ...
                       try {
                           ...
                           auto jsonResponse = nlohmann::json::parse(response_string);
                           
                           // Check if the response contains choices and text
                           if (jsonResponse.contains("choices") && jsonResponse["choices"].is_array()) {
                               generated_text = jsonResponse["choices"][0]["message"]["content"].get<std::string>(); // Adjusted for the new response structure
                           } else {
                               generated_text = "I'm sorry, I didn't understand that. Can you please rephrase?";
                           }
    ...
    else if(chooseBot ==-1 ||chooseBot ==-2) {
        CURL* curl;
          CURLcode res;
          std::string response_string;
          struct curl_slist* headers = NULL;
          curl = curl_easy_init();
          std::string translated_text = "Error: Unable to process the request.";
          
          if (curl) {
              const std::string api_key = "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"; // Replace with your OpenAI API key
              const std::string api_url = "https://api.openai.com/v1/chat/completions"; // API endpoint
              std::string translateString ="Translate the following text to Turkish : ";
              std::string fullTranslateString =translateString.append(prompt);
              // Create the JSON payload
              nlohmann::json jsonData;
              jsonData["model"] = "gpt-3.5-turbo"; // Use gpt-3.5-turbo model
              if(chooseBot ==-1 ){
                  jsonData["messages"] = {
                      ...
                      {{"role", "user"}, {"content", fullTranslateString}}
                  };
              }
              else if(chooseBot ==-2)
              {
                  jsonData["messages"] = {
                             ...
                             {{"role", "user"}, {"content", "You are an assistant that detects whether a sentence is in English or Turkish. The user may write with spelling mistakes, so make a best guess. If the following sentence is in English or a close variation, respond with the text 'English', otherwise respond with 'Not-English':" + prompt}}
                         };
                  std::cout<<prompt<<std::endl;
              }
              jsonData["max_tokens"] = 100;
              ...
              if (res != CURLE_OK) {
                  ...
                  try {
                      ...
                      // Check if the response contains choices and text
                      if (jsonResponse.contains("choices") && jsonResponse["choices"].is_array()) {
                          translated_text = jsonResponse["choices"][0]["message"]["content"].get<std::string>();
                      } else {
                          translated_text = "I'm sorry, I didn't understand that. Can you please rephrase?";
                      }
                  } catch (const nlohmann::json::exception& e) {
                      // Handle JSON parsing errors
                      std::cerr << "JSON Parsing Error: " << e.what() << std::endl;
    ...
    if(curl) {
        const std::string api_key = "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"; // Replace with your Hugging Face API key
        const std::string api_url = "https://api-inference.huggingface.co/models/facebook/blenderbot-400M-distill";
        ...
        if(res != CURLE_OK) {
            ...
            try {
                ...
                if (!jsonResponse.empty() && jsonResponse.is_array() && jsonResponse[0].contains("generated_text")) {
                    generated_text = jsonResponse[0]["generated_text"].get<std::string>();
                } else {
                    generated_text = "I'm sorry, I didn't understand that. Can you please rephrase?"; //Meta3D: Error: Unexpected response format."
                }
            } catch (const json::exception& e) {
                // Handle JSON parsing errors
                std::cerr << "JSON Parsing Error: " << e.what() << std::endl;
                generated_text = "I'm sorry, I didn't understand that. Can you please rephrase?"; //Meta3D: Error: JSON parsing failed.
            }
        }
        ...
        generated_text = "I'm sorry, I didn't understand that. Can you please rephrase?"; //Meta3D: Error: CURL initialization failed."
    }
...
wxString ChatFrame::CallChatbotAPI(const wxString& userText) {
    ...
    if(botResponse == "I'm sorry, I didn't understand that. Can you please rephrase?")
    {
        ...
        botResponse = wxString(queryAdditionalChatbot(std::string(userText.mb_str()), 0 ));
        
    }
    else
    {
        ifRasa = true;
        //botResponse =  wxString(queryAdditionalChatbot(std::string(botResponse.mb_str()), -1 ));
    }
      ...
      connectionStatus(Disconnected)  // Now 'priv' is a complete type
{
    ...
    AddObserver(new QualityObserver());
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(255,174,0), StateColor::Pressed),
...
//        view3D_canvas->Bind(EVT_GLTOOLBAR_DELETE_ALL, [q](SimpleEvent&) { q->reset_with_confirm(); });
    ...
    if (wxGetApp().is_editor()) {
        ...
        q->Bind(EVT_IDEX_COPY, &priv::on_action_idex_copy, this);
...
std::vector<size_t> Plater::priv::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi, int index, bool load_to_idex_machine)
{
   ...
   //
       ...
       // Don't make a copy on Posix. Slash is a path separator, back slashes are not accepted as a substitute.
        ...
        try {
            if (type_3mf) {
                ...
                bool                  is_xxx;
                Semver                file_version;
                //Meta3D: Read model from path for stl files
    ...
    if (new_model != nullptr && new_model->objects.size() > 1) {
        ...
        //load_model_objects(new_model->objects);
        obj_idxs.insert(obj_idxs.end(), loaded_idxs.begin(), loaded_idxs.end());
    ...
    if (!obj_idxs.empty() && view3D != nullptr) {
        ...
        //Meta3D: Make prepare models shadowy when print is closed on objects
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z, bool split_object, LoadStrategy strategy,int index)
{
   ...
   // std::cout <<"I am at load_model_objects model_objects size "<< model_objects.size() <<std::endl;
    for (ModelObject *model_object : model_objects) {
        ...
        std::cout<<"Object in loop"<<std::endl;
        if(strategy == LoadStrategy::IdexCopy||strategy == LoadStrategy::IdexMirror)
        {
            std::cout<<"Object printable is set to falser"<<std::endl;
            object->printable=false;
        }
        std::cout<<model_objects.size()<<std::endl;
        ...
        for (size_t i = 0; i < object->instances.size(); ++i) {
            ...
            //object->volumes[i]->source.volume_idx=43;
        }
    ...
    for (auto& instance : new_instances) {
        ...
        //copy_instances.back()->set_offset(250 - q->copy_instances.front()->get_offset().x(), q->copy_instances.front()->get_offset().y(),offset(2));
        instance->set_offset(displacement);
        //offset_vector.emplace_back({3,3,3});
        Vec3d xd = displacement;
        //offset_vector.emplace_back(xd);
        //(q->copy_instances).emplace_back(instance);
    }
#endif
...
void Plater::priv::set_current_panel(wxPanel* panel, bool no_slice)
{
    ...
    if (panel == view3D || panel == preview) {
        ...
        this->enable_chatframe(!q->only_gcode_mode());
    }
    if (panel == preview) {
...
void Plater::priv::on_select_preset(wxCommandEvent &evt)
{
    ...
    if (preset_type == Preset::TYPE_FILAMENT && sidebar->is_multifilament()) {
        ...
        std::string preset_name_temp = get_preset_name();
       
        if(preset_name_temp == "Meta3D XPro - MIRROR")
        {
            std::cout<< "I am at on_select_preset() MIRROR"<<std::endl;
            load_idex(LoadStrategy::IdexMirror);
            m_strategy = LoadStrategy::IdexMirror;
        }
        else if(preset_name_temp == "Meta3D XPro - COPY"){
        ...
        if(!(preset_name_temp == "Meta3D XPro - COPY" ||preset_name == "Meta3D XPro - MIRROR"))
...
std::string Plater::priv::get_preset_name()
{
    const DynamicPrintConfig& printer_config = wxGetApp().preset_bundle->printers.get_edited_preset().config;
    std::string preset_name = "";
    auto* printer_model_option = printer_config.option<ConfigOptionString>("printer_model");

    if (printer_model_option != nullptr) {
        preset_name = printer_model_option->value;
        std::cout<<"Preset name is "<<preset_name<<std::endl;
    } else {
        std::cerr << "Failed to retrieve printer_model option" << std::endl;
    }
...
{
    update_chatframe();
    //wxMessageBox("This is the information screen.", "Information", wxOK | wxICON_INFORMATION, this);
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/logo_360.ico") % resources_dir()).str();
    ...
    m_confirm->SetBorderColor(wxColour(255,174,0));
