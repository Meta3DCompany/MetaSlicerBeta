...
public:
    ...
    void Decorate(wxBitmap& bmp)
    {
        ...
        memDc.DrawLabel("MetaSlicer Ver. Beta", version_rect, wxALIGN_CENTER);
        ...
        auto bs_version = wxString::Format("MetaSlicer is based on PrusaSlicer, BambuStudio and OrcaSlicer").ToStdString();
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #FFAE00   -- 73
        if (bg_col != orig_col) {
...
{
    //Meta3D: The config wizard is shut down will be open soon
    if (false) {
